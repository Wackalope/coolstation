...
/obj/item/spraybottle/cleaner/robot/lube
	name = "cybernetic lubricant spray bottle"
	desc = "A cleaner spray bottle jury-rigged to synthesize space lube."
	icon_state = "cleaner_robot"

	New()
		..()
		reagents.add_reagent("lube", 25)

	process()
		..()
		if (src.reagents.total_volume < 25)
			src.reagents.add_reagent("lube", 1)
		else
			processing_items.Remove(src)
		return 0
